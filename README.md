...
$$h_i^{(l+1)} = \text{LayerNorm}\left(\sigma\left(\Vert_{k=1}^K \sum_{j \in \mathcal{N}_i} \alpha_{ij}^{k, r} \mathbf{W}_{\text{dst}}^k h_j^{(l)}\right) + \mathbf{W}_{\text{res}} h_i^{(l)}\right)$$
